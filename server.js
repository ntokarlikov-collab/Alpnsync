...
// Deep Multi-Accommodation Database Array
const baseLodgingRegistry = {
    ...
    "Zermatt": [
        { id: "z1", name: "Zermatt Matterhorn Mountain Lodge", feature: "Ski locker assemblies, close to rail station hubs", price: 75, img: "https://images.unsplash.com/photo-1555854877-bab0e564b8d5?auto=format&fit=crop&w=300&q=80" },
        { id: "z2", name: "The Omnia Premium Alpine Peak", feature: "Luxury view decks matching Matterhorn ridge paths", price: 280, img: "https://images.unsplash.com/photo-1540555700478-4be289fbecef?auto=format&fit=crop&w=300&q=80" }
    ],
    "Verbier": [
    ...
    "Grindelwald": [
        ...
        { id: "g2", name: "Belvedere Alpine Resort", feature: "Thermal mountain-view saltwater outdoor springs", price: 195, img: "https://images.unsplash.com/photo-1571896349842-33c89424de2d?auto=format&fit=crop&w=300&q=80" }
    ]
};
...
app.post('/api/compute-expedition', async (req, res) => {
    try {
        ...
        } catch(e) { console.log("SBB OpenData API Timeout fallback activated"); }
        ...
        let liftTicketStatusText = passSystem !== "None" ? `${passSystem} Card Applied` : `CHF ${(liftBaseEach * travelers).toFixed(2)} Base Lift Fee`;

        // Load deep matching accommodations matching choice metrics
        let hotels = baseLodgingRegistry[destKey] || baseLodgingRegistry["Andermatt"];
        let calculatedHotels = hotels.map(h => ({
            id: h.id, name: h.name, feature: h.feature, img: h.img, price: (h.price * travelers).toFixed(2)
        }));
...
app.listen(PORT, () => console.log(`AlpenSync Enterprise Infrastructure active on port ${PORT}`));
